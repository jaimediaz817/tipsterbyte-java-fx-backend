...
- El repo está en **FASE 8 completada (PostgreSQL + JPA)**. Modelo de dominio completo en `domain.model` (enums, VOs, entities, aggregates `Liga/Partido/Pronostico/Suscripcion` con BR-001..008), eventos en `domain.event` + interfaz `DomainEvent`, factory methods `reconstruir(...)` (sin emitir eventos) para hidratar desde BD. Capa application con los 9 casos de uso (CU-01..09) en `application.usecase`, puertos `ProveedorPosiciones/Calendario/Cuotas` + 4 repositorios en `application.port`, DTOs en `application.dto`. Capa interfaces con 4 controllers (`interfaces.rest.controller`), request/response DTOs y `GlobalExceptionHandler` (DomainException → 422, validación → 400). Capa infrastructure con 7 entidades JPA (`infrastructure.persistence.entity`), 4 repositorios Spring Data (`infrastructure.persistence.repository`) y 4 adapters JPA (`infrastructure.adapter`: `LigaRepositoryJpaAdapter`, `PartidoRepositoryJpaAdapter`, `PronosticoRepositoryJpaAdapter`, `SuscripcionRepositoryJpaAdapter`). 114 tests en verde (incl. 11 de integración con Testcontainers). Los controllers se registran solo con `app.api.rest.enabled=true` (sigue en `false`; el wiring REST completo se difiere a FASE 8.5/17 porque CU-01..04 requieren los proveedores). FASE 8 **sin commitear**.
...
- Próxima: FASE 8.5 (Adapters de fuentes externas). El usuario tiene 4 endpoints de extracción funcionando (ligas/torneos por país — incluye el endpoint "ligas por país" que mencionó el usuario como fuente #1, tablas de posiciones con últimos 5 resultados por equipo, calendario con estadísticas, cuotas de Wplay) que alimentarán los puertos del modelo. **Antes de implementar cualquier adapter, pedir al usuario la respuesta real (JSON) del endpoint y el detalle de qué expone; NO asumir formatos.**
