...
//        (Soccerway). Mapea equipos, fechaHora y jornada (label "Jornada N").
...
public class SoccerwayCalendarioAdapter implements ProveedorCalendario {
    ...
    public List<PartidoFuente> obtenerCalendario(UUID ligaId) {
        ...
        try {
            ...
            // [POR QUÉ]: FASE 8.5 solo crea partidos (equipos + fechaHora + jornada). Los
            //            goles, estado FINALIZADO y estadísticas quedan para un futuro CU de
            ...
            for (List<PartidoJson> jornada : respuesta.partidosPorJornada()) {
                ...
                for (PartidoJson partido : jornada) {
                    partidos.add(new PartidoFuente(
                            ...
                            parsearJornada(partido.jornada(), indiceJornada)));
                }
                indiceJornada++;
            }
            return partidos;
    ...
    // [QUÉ]: Extrae el número de la jornada del label real de #4 ("Jornada 4" → 4).
    // [POR QUÉ]: El frontend necesita la jornada como Integer para el indicador
    //            cronológico. Si el label no trae número, se usa el índice del array
    //            partidos_por_jornada como fallback robusto.
    // [ALTERNATIVAS]: Confiar solo en el label; se descarta porque un label inesperado
    //                 dejaría la jornada nula (mejor el índice ordinal).
    private Integer parsearJornada(String label, int indiceJornada) {
        if (label != null) {
    ...
    private record PartidoJson(
            ...
            @JsonProperty("jornada") String jornada) {
    }
}
