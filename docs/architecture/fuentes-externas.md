...
- **Ejecución programada por fuente (FASE 12.5, definida — no ejecutada)**: cada `DetalleFuenteExtraccion` (liga + tipo) podrá tener una **tarea programada** asociada (`tareas_programadas`, FK a `detalle_fuentes_extraccion`); además existe una tarea global de catálogo (CU-10) sin detalle. El `CatalogoScheduler` evaluará el cron de cada tarea activa y disparará el adapter según el tipo: `CALENDAR` → `SoccerwayCalendarioAdapter` (vía `SincronizarCalendarioUseCase`), `ODDS_WPLAY` → `WplayCuotasAdapter` (vía `SincronizarCuotasUseCase`), `STANDINGS` → `FlashscorePosicionesAdapter` (vía `SincronizarPosicionesUseCase`). Frecuencias objetivo: cuotas cada hora, calendario cada 8 días, posiciones diario, catálogo semanal. Detalle y comunicado frontend en `docs/PROYECTO-PLAN.md` (FASE 12.5).
